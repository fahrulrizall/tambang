...
const { Remarks } = require("../db/models");
...
const pagedSearcRemarks = async (req, res) => {
  ...
  try {
    ...
    const result = await Remarks.findAll({
...
const createNewRemakrs = async (req, res) => {
  const errros = validationResult(req);
...
const readRemarks = async (req, res) => {
  const { uuid } = req.params;

  try {
    const result = await Remarks.findOne({
      where: {
        uuid: uuid,
      },
      raw: true,
